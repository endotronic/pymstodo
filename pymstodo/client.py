...
class Task:
    ...
    def created_date(self) -> Optional[datetime]:
        if self.createdDateTime:
            return datetime.strptime(
                self.createdDateTime.split(".")[0], "%Y-%m-%dT%H:%M:%S"
            )
        else:
            return None
    ...
    def last_mod_date(self) -> Optional[datetime]:
        if self.lastModifiedDateTime:
            return datetime.strptime(
                self.lastModifiedDateTime.split(".")[0], "%Y-%m-%dT%H:%M:%S"
            )
        else:
            return None
...
class ToDoConnection:
    ...
    def get_lists(self, limit: Optional[int] = 99) -> Optional[List[TaskList]]:
        ...
        self._refresh_token()
        oa_sess = OAuth2Session(
            self.client_id, scope=ToDoConnection._scope, token=self.token
        )
    ...
    def create_list(self, name: str) -> Optional[TaskList]:
        ...
        resp = oa_sess.post(
            f"{ToDoConnection._base_api_url}lists", json={"displayName": name}
        )
        if not resp.ok:
            raise PymstodoError(f"Error {resp.status_code}: {resp.reason}")
    ...
    def update_list(
        self, list_id: str, **list_data: Union[str, bool]
    ) -> Optional[TaskList]:
        ...
        resp = oa_sess.patch(
            f"{ToDoConnection._base_api_url}lists/{list_id}", json=list_data
        )
        if not resp.ok:
            raise PymstodoError(f"Error {resp.status_code}: {resp.reason}")
    ...
    def get_tasks(
        self, list_id: str, limit: Optional[int] = 99, status: Optional[str] = None
    ) -> Optional[List[Task]]:
        ...
        if status:
            if status == "all":
                resp = oa_sess.get(
                    f"{ToDoConnection._base_api_url}lists/{list_id}/tasks?$top={limit}"
                )
            elif status == "completed":
                resp = oa_sess.get(
                    f"{ToDoConnection._base_api_url}lists/{list_id}/tasks?$top={limit}&$filter=status eq 'completed'"
                )
            elif status == "notCompleted":
                resp = oa_sess.get(
                    f"{ToDoConnection._base_api_url}lists/{list_id}/tasks?$top={limit}&$filter=status ne 'completed'"
                )
            else:
    ...
    def create_task(
        ...
        resp = oa_sess.post(
            f"{ToDoConnection._base_api_url}lists/{list_id}/tasks", json=task_data
        )
        if not resp.ok:
            raise PymstodoError(f"Error {resp.status_code}: {resp.reason}")
    ...
    def get_task(self, task_id: str, list_id: str) -> Optional[Task]:
        ...
        resp = oa_sess.get(
            f"{ToDoConnection._base_api_url}lists/{list_id}/tasks/{task_id}"
        )
        if not resp.ok:
            raise PymstodoError(f"Error {resp.status_code}: {resp.reason}")
    ...
    def update_task(
        self, task_id: str, list_id: str, **task_data: Union[str, int, bool]
    ) -> Optional[Task]:
        """Update task details."""
        self._refresh_token()
        oa_sess = OAuth2Session(
